...
function loop_smb(microBolusAllowed, profile, iob_data, aimismb, iobTH_reduction_ratio) {
    ...
    if ((profile.use_autoisf && profile.temptargetSet && profile.enableSMB_EvenOn_OddOff) || (profile.use_autoisf && profile.min_bg==profile.max_bg && profile.enableSMB_EvenOn_OddOff_always && !profile.temptargetSet))  {
        ...
        if ( !evenTarget ){
            ...
            if (profile.temptargetSet && target < 100) {     // indirect asessment; later set it in GUI
                console.error("iobTH: " + round(iobTHeffective,1) + "%, IOB% of maxIOB at " + round(iob_data.iob/(profile.max_iob*iobTH_reduction_ratio)*100,1) + "%")
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, oref2_variables) {
   ...
   const tdd_ytd = oref2_variables.tddYtd;
   const average_7days = oref2_variables.tdd7d;
   ...
   if (pumpData > 21) {
       ...
       tddReason = ", TDD, 24h: " + round(tdd,1) + ", ytd: " + round(tdd_ytd,1)+ ", 7d\u00D8: " + round(average_7days,1);
...
// Cancel high temps (and replace with neutral) or shorten long zero temps for various error conditions
    ...
    //cherry pick from oref upstream dev cb8e94990301277fb1016c778b4e9efa55a6edbc
    if (bg <= 10 || bg === 38 || noise >= 3 || minAgo > 12 || minAgo < -5 || ( bg > 60 && glucose_status.cgmFlatMinutes > cgmWaitLimit ) ) {
...
// Print summary variables based on predBGs etc.
    ...
    rT.TDDytd=round(tdd_ytd, 1);
    rT.TDD7d=round(average_7days, 1);
    ...
    rT.reason =  profile.mw_Reason + B30reason + SMBdelreason + TTreason + isfreason + ", Standard, COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI+ ", ISF: " + rT.ISF + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
